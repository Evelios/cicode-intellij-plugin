...
final class CicodeSpellcheckingStrategy extends SpellcheckingStrategy {

  @Override
  public @NotNull Tokenizer<?> getTokenizer(PsiElement element) {
    if (element instanceof PsiComment) {
      return new CicodeCommentTokenizer();
    }

    if (element instanceof CicodeProperty) {
      return new CicodePropertyTokenizer();
    }

    return EMPTY_TOKENIZER;
  }

  private static class CicodeCommentTokenizer extends Tokenizer<PsiComment> {
  ...
  private static class CicodePropertyTokenizer extends Tokenizer<CicodeProperty> {

    public void tokenize(@NotNull CicodeProperty element, TokenConsumer consumer) {
      //Spell check the keys and values of properties with different splitters
      final ASTNode key = element.getNode().findChildByType(CicodeTypes.KEY);
      ...
      final ASTNode value = element.getNode().findChildByType(CicodeTypes.VALUE);
